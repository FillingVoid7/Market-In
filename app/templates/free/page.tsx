...
const FreeTemplate: React.FC = () => {
    ...
    const handleProductImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            const files: ImageFile[] = Array.from(e.target.files).map((file) => ({
                file,
    ...
    const handleShopImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            dispatch(updateShopImages([URL.createObjectURL(file)]));
        }
    };
    ...
    const generateUniqueURL = () => {
        ...
        const newUrl = `https://ReachTotal.com/product/${Math.random().toString(36).substr(2, 9)}`;
        ...
        <div className="flex justify-center items-center mt-14 w-screen min-h-screen">
            <div className="space-y-6">
                            ...
                            <div key={faq.id} className="border rounded-lg p-4">
                                {faq.isEditing ? (
                                    ...
                                    <>
                                        ...
                                        >
                                            <p className="font-semibold mb-2">{index + 1}. {faq.question}</p>
                                            <span
                ...
                <div className="mt-6">
                    ...
                    >
                        Generate Unique URL {uniqueURLs.length}/3
                    </button>
                    {uniqueURLs.length > 0 && (
