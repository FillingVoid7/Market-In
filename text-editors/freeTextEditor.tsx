...
const FreeTextEditor: React.FC<FreeTextEditorProps> = ({
  value = "",
  onSave,
  placeholder,
}) => {
    ...
    return (
      <div className="mt-2">
        {content ? (
          <div
            style={style}
            className="p-3 border rounded-lg mb-2 min-h-[100px]"
          >
            {content}
          </div>
        ) : null}
    ...
    <div className="border rounded-lg p-3 mt-2 shadow-sm text-black">
      ...
      <div className="flex gap-3 mb-2 flex-wrap pb-2">
        ...
        <div className="flex gap-2 items-center border-r pr-3">
          ...
          />
          <ToolbarButton
            onClick={() => handleListFormat("numbered")}
            icon={ListOrdered}
            title="Numbered List"
          />
        </div>
        ...
        <div className="flex gap-2 items-center border-r pr-3">
          ...
          <select
            onChange={(e) =>
              setStyle((prev) => ({ ...prev, lineHeight: e.target.value }))
            }
            className="border rounded p-1 text-sm bg-white"
